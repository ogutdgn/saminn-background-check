...
- [ ] Open: Dallas latency tuning · Dallas name/DOB parser · `IMAGE_SOURCES`→backend flag · **pre-search card placeholder shows "Odcr County" (titleCase(id)+" County") — wrong for a statewide source; have `/api/health` return display names** · merge `source/odcr`+`source/hunt`+everything-branch to `main` · **next source: Denton (T3) or Collin (T4 → `browser.py`)**.
