...
| **Denton County, TX** | Tyler Public Access, *self-hosted* (`justice1.dentoncounty.gov`) | 3 | http | Cloudflare (passes this IP) | — | 🟢 proven (flow cracked; 0-record blocker) | in-progress |
...
- **Denton** — The only *self-hosted* Tyler instance we use. Self-hosted Tyler works;
  ...
  **JP & County Criminal** name search (`Search.aspx?ID=100`): portal → POST `NodeID`
  ...
  `CaseSearchResults.aspx`. **Open blocker:** the search returns 0 records until the
  AJAX/embedded **`CaseTypeIDs`** (case categories) is populated from the node-aware
  form — next step. Full detail: `backend/tests/fixtures/denton/README.md`. Also worth
  adding later: the **District Court** module (`../PublicAccessDC/Search.aspx?ID=200`)
  for felonies.
